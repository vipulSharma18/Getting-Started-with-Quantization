...
def model_quantize(causal_model, quantized=False):
    if not quantized:
        patch_model(causal_model.model, device="cuda", processor=A4W4_NVFP_dynamic, skip_modules=[])
        torch.cuda.empty_cache()
        gc.collect()

        # patch_model(causal_model.lm_head, device="cuda", processor=A4W4_NVFP_dynamic, skip_modules=[])
        # torch.cuda.empty_cache()
        # gc.collect()
    else:
        pass
