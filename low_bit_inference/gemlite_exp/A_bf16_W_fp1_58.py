...
monkeypatch_gemlite()
from gemlite.helper import A16W158_INT
...
def model_quantize(causal_model, quantized=False):
    if not quantized:
        patch_model(causal_model.model, device="cuda", processor=A16W158_INT, skip_modules=[])
        torch.cuda.empty_cache()
        gc.collect()

        # patch_model(causal_model.lm_head, device="cuda", processor=A16W158_INT, skip_modules=[])
        # torch.cuda.empty_cache()
        # gc.collect()
    else:
        pass
