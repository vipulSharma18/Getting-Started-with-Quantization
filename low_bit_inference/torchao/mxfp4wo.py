import torch
from torchao.quantization import quantize_, FPXWeightOnlyConfig  # using NVFP4
...
def model_quantize(causal_model, quantized=False):
    if not quantized:
        quantize_(causal_model.model, FPXWeightOnlyConfig(ebits=3, mbits=2))
        quantize_(causal_model.lm_head, FPXWeightOnlyConfig(ebits=3, mbits=2))
    else:
        pass
